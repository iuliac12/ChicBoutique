...
class AccesBD{
    ...
    constructor() {
        ///ne asiguram ca avem o singura instanta
        if(AccesBD.#instanta){
    ...
    ///nu avem param numiti in js => definim un obiect cu proprietati
    static getInstanta({init="local"}={}){
        console.log(this);//this-ul e clasa nu instanta pt ca metoda statica
        if(!this.#instanta){
            this.#initializat=true;
            this.#instanta=new AccesBD(); ///cream instanta
            ...
            try{
                switch(init){
                    case "local":this.#instanta.initLocal(); ///conexiunea la BD
                }
        ...
        return this.#instanta; ///returneaza o instanta
    }
    ...
    select({tabel="",campuri=[],conditiiAnd=[]} = {}, callback, parametriQuery=[]){
        ...
        if(conditiiAnd.length>0)
            conditieWhere=`where ${conditiiAnd.join(" and ")}`; ///where nume = .. and culoare = .. and
...
module.exports=AccesBD; ///modul propriu => ce anume din fisier vreau sa export 